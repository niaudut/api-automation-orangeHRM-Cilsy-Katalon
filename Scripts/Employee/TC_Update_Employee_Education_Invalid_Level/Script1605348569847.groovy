...
response = WS.sendRequestAndVerify(findTestObject('Employee/GET_Employee_Education', [('url') : GlobalVariable.url, ('token') : GlobalVariable.token]))

def slurper1 = new groovy.json.JsonSlurper()

def result1 = slurper1.parseText(response.getResponseBodyContent())

def seqId = result1.data[0].seqId
...
WS.sendRequestAndVerify(findTestObject('Employee/PUT_Employee_Education_Invalid_Level', [('url') : GlobalVariable.url, ('token') : GlobalVariable.token]))
